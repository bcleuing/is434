...
function getPageFanInfo(pageName, pageId, accessToken) {
    FB.api("/" + pageId + "/insights/page_fans_gender_age", {access_token: accessToken}, function(response) {
        ...
        if (values != null) {
            ...
            $('#pagesFanInfo').append("<div class='row'><div class='col s12 card-panel' style='margin-top: 30px;'><h4 class='center-align blue-text'>" + pageName + "</h4></div></div>");
            var formattedStat = {
            ...
            var canvasId = pageId + "-canvas-" + (count++); // generate unique id for canvas
            $('#pagesFanInfo').append("<div class='col s12'><h5>Number of Fans (Overall)</h5>'<canvas id='" + canvasId + "'></canvas></div>");
            var data = {
                labels: ageGroupLabels,
                datasets: [
                    {
